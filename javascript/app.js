class Ship {
  ...
  attackShip(enemy){
    if(this.shotResult()){
      ...
      myText += "<br>";
      myText += enemy.name + " has " + enemy.hull + " Armor left.";
      return myText;
      // <img src= "img/destroyed.jpg">;
      // $("#destroyed").attr('src',"img/destroyed.jpg");
    } else {
...
const combat = () => {
  ...
  while(!myShip.destroyed() && !alienShips[0].destroyed()){


    pushText(myShip.attackShip(alienShips[0]));


    if(!alienShips[0].destroyed()){
